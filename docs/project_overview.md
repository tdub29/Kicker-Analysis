...
## Why this structure
This project uses a data science layout with clear data stages, code ownership, and documented workflows.
Each top-level directory has a defined purpose and scope.

### Top-level directories (with purpose)
- **configs/**: Centralized configuration for data sources, season ranges, and pipeline settings. Keeps parameters versioned and consistent across runs.
...
- **notebooks/**: Exploratory analysis and ad-hoc investigations. Kept separate from pipeline code.
...
1. **Project overview** — problem statement and data sources.
...
- Keep notebooks clean; export stable logic to `src/` once it is reviewed.
