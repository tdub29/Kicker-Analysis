...
This project builds a production-ready analytics pipeline to forecast NFL kicker outcomes, including expected field goal attempts, fantasy points, and market-style thresholds (e.g., 1.5 made field goals). It combines rolling team performance, opponent defensive context, and schedule information to create an audience-facing dataset that can power weekly decision-making and model development.

The repo now includes a full Python replica of the original R workflow, with a modular pipeline that is reproducible, configurable, and ready for collaboration.
...
- Provide a structure that is stable for future contributors and production usage.
